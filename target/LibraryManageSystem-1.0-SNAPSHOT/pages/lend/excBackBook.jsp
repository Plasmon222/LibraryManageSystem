...
<div class="layui-form layuimini-form">
    <input type="hidden" name="id"  value="${lendid}"/>
<%--    <input type="hidden" name="bookId"  value="${lend.bid}"/>--%>
...
<script>
    layui.use(['form', 'table'], function () {
        var $ = layui.jquery,
            form = layui.form,
            table = layui.table;
        let localCategory = JSON.parse(localStorage.getItem("lendid"))
        //监听提交
        form.on('submit(saveBtn)', function (data) {
            var datas=data.field;//form单中的数据信息
            //向后台发送数据提交添加
            $.ajax({
                url:"excBackBookController",
                type:"POST",
                data:datas,
